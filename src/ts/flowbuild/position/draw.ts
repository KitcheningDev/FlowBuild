import { Vec2, vec2Add, vec2Sub } from "../../utils/vec2.js";
import { MetricGrid } from "./metric_grid.js";
import { Node } from "../graph/node.js";
import { FlowGrid } from "../grid/flow_grid.js";
import { Line, LineDir, Tile } from "../grid/tile.js";
...
import { button_tile, cookline_tile, makeCookTitle, syncline_tile } from "./html_size.js";
...
function drawElement(el: HTMLElement, metric_tile: MetricTile, size?: Vec2): void {
...
// task
function createTask(node: Node): HTMLElement {
    if (node.isStart()) {
...
// editor
function createConnect(node: Node): HTMLElement {
...
export function draw_grid(flow_grid: FlowGrid, metric_grid: MetricGrid, graph: Graph): void {
    ...
    for (const [tile, coords] of flow_grid.entries) {
        ...
        // }
        // lines
        for (const dir of ['top', 'right', 'bottom', 'left'] as LineDir[]) {
            if (tile.lines[dir] == 'out') {
                if (dir == 'top') {
                    const offset = metric_grid.maxTop(coords, true);
                    drawElement(createLine('top'), new MetricTile(vec2Add(metric.pos, new Vec2(0, -offset / 2))), new Vec2(1, offset));
                }
                else if (dir == 'right') {
                    const offset = metric_grid.maxRight(coords, true);
                    drawElement(createLine('right'), new MetricTile(vec2Add(metric.pos, new Vec2(offset / 2, 0))), new Vec2(offset, 1));
                }
                else if (dir == 'bottom') {
                    const offset = metric_grid.maxBottom(coords, true);
                    drawElement(createLine('bottom'), new MetricTile(vec2Add(metric.pos, new Vec2(0, offset / 2))), new Vec2(1, offset));
                    ...
                    const offset = metric_grid.maxLeft(coords, true);
                    drawElement(createLine('left'), new MetricTile(vec2Add(metric.pos, new Vec2(-offset / 2, 0))), new Vec2(offset, 1));
                }
            }
        }
        // cook title
        if (tile.cook_title) {
            drawElement(makeCookTitle(tile.cook_title), metric);
    ...
    for (const [node, _] of flow_grid.nodeEntries) {
        if (node.hasTopSyncline) {
            const bounds = flow_grid.bounds((other: Node) => node.topSyncline.has(other));
        ...
        if (node.hasBottomSyncline) {
            const bounds = flow_grid.bounds((other: Node) => node.bottomSyncline.has(other));
