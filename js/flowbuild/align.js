...
export class Alignment {
    constructor(members, type) {
        ...
        if (type == 'hor') {
            ...
            this.in = members[0].in;
            this.out = members[members.length - 1].out;
            this.parents = members[0].parents;
            this.childs = members[members.length - 1].childs;
        }
        else if (type == 'path') {
            this.in = new Set([members[0]]);
            this.out = new Set([members[members.length - 1]]);
            this.parents = set_merge(this.parents, members[0].parents);
            this.childs = set_merge(this.childs, members[members.length - 1].childs);
        }
    }
    to_html() {
        const div = create_div('align-' + this.type);
        if (this.type == 'path') {
            if (!this.members[0].is_start()) {
...
;
export function create_paths(graph) {
    ...
    for (const [node, set] of relation_map) {
        if (!visited.has(set)) {
            paths.add(new Alignment([...set].sort((a, b) => graph.get_depth(a) - graph.get_depth(b)), 'path'));
            visited.add(set);
        }
    }
    return paths;
}
export function hor_merge(ver_aligns, rigorous = true) {
    const relation_map = new Map();
...
export function ver_merge(hor_aligns, rigorous = true) {
    const relation_map = new Map();
