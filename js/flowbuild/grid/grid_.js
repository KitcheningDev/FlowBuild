...
var _Grid_set_function, _Grid_default_constructor, _Grid_data, _Grid_size, _Grid_val_map;
...
export class Grid {
    constructor(set_function, default_constructor, size = new Vec2(0, 0)) {
        ...
        __classPrivateFieldSet(this, _Grid_val_map, new Map, "f");
        this.set_size(size);
    }
    // access
    in_bounds(coords) {
    ...
    set(val, coords) {
        if (this.in_bounds(coords)) {
            __classPrivateFieldGet(this, _Grid_set_function, "f").call(this, val, coords, this);
    ...
    get_entries() {
        ...
        for (let y = 0; y < __classPrivateFieldGet(this, _Grid_size, "f").y; ++y) {
            for (let x = 0; x < __classPrivateFieldGet(this, _Grid_size, "f").x; ++x) {
                entries.add(new IEntry(this.get(new Vec2(x, y)), new Vec2(x, y)));
            }
        }
        return entries;
    }
    // coords map
    has_val_map(val) {
        return __classPrivateFieldGet(this, _Grid_val_map, "f").has(val);
    }
    set_val_map(val, coords) {
        if (this.in_bounds(coords)) {
            __classPrivateFieldGet(this, _Grid_val_map, "f").set(val, coords.copy());
        }
        console.error("invalid input", val);
    }
    get_val_map(val) {
        if (__classPrivateFieldGet(this, _Grid_val_map, "f").has(val)) {
            return __classPrivateFieldGet(this, _Grid_val_map, "f").get(val);
        }
        console.error("invalid input", val);
    }
    remove_val_map(val) {
        __classPrivateFieldGet(this, _Grid_val_map, "f").delete(val);
    }
    get_val_map_entries() {
        return __classPrivateFieldGet(this, _Grid_val_map, "f").entries();
    }
    // size
    is_empty() {
    ...
    set_size(size) {
        ...
        __classPrivateFieldSet(this, _Grid_val_map, __classPrivateFieldGet(grid, _Grid_val_map, "f"), "f");
...
_Grid_set_function = new WeakMap(), _Grid_default_constructor = new WeakMap(), _Grid_data = new WeakMap(), _Grid_size = new WeakMap(), _Grid_val_map = new WeakMap();
//# sourceMappingURL=grid_.js.map