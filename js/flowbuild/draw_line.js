...
function get_center_x(paths, origin_map) {
...
function draw_global_line(path, paths, origin_map, entry_map) {
...
function create_entry_map(paths, ignore, relative) {
    ...
    for (const path of paths) {
        ...
        if (!found_entry) {
            const entry = { cook_id: path.cook_id, relatives: path[relative], paths: new Set([path]) };
            entry_map.set(path.id, entry);
        }
    }
    return entry_map;
}
function get_in_pos(path, origin_map, in_map) {
...
function get_out_pos(path, origin_map, out_map) {
