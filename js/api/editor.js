...
let from_connector = null;
let to_connector = null;
let from_task = null;
let to_task = null;
...
chart.addEventListener('mousedown', (e) => {
    ...
    if (e.target.classList.contains("editor-connector-from")) {
        from_connector = e.target;
        ...
        root.style.setProperty('--editor-opacity', '0');
        from_task = recipe.get_task_by_id(parseInt(e.target.id));
        console.log("mousedown", from_task);
    }
});
document.addEventListener('mouseup', (e) => {
    console.log(e.target);
    if (e.target.classList.contains("editor-connector-to")) {
        to_connector = e.target;
