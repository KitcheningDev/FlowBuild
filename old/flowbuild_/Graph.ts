...
export class Graph {
    ...
    private CreateNodeMap(connections: Connection[]): void {
        for (const connection of connections) {
            if (!this.node_map.has(connection[0]))
                this.node_map.set(connection[0], { text: connection[0], parents: [], childs: [] });
            if (!this.node_map.has(connection[1]))
                this.node_map.set(connection[1], { text: connection[1], parents: [], childs: [] });

            if (!this.node_map.get(connection[1]).parents.includes(connection[0]))
                this.node_map.get(connection[1]).parents.push(connection[0]);
            if (!this.node_map.get(connection[0]).childs.includes(connection[1]))
                this.node_map.get(connection[0]).childs.push(connection[1]);
        }
    }
    ...
    private RecursiveAdd(head: string): Path {
        ...
        if (head != "START") {
            let child_node = null;
            while (curr_node.childs.length == 1 && curr_node.text != "END") {
    ...
    private SetDepthDiff(): void {
        for (const path of this.path_map.values()) {
            ...
            for (const child of path.childs)
                path.advanced.depth_diff = Math.max(path.advanced.depth_diff, child.advanced.depth - path.advanced.depth);
        }
    }
