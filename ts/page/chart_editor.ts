...
import { load_recipe, cache_recipe_mod, recipe } from "./load_recipe.js";
...
let selected_task_html: HTMLElement = null;
...
const duration_input_html = document.getElementById("task-duration-input") as HTMLInputElement;
...
export function start_task_form(el: HTMLElement) {
...
export function stop_task_form(proceed: boolean) {
...
chart_html.addEventListener('mouseup', (e: MouseEvent) => {
    ...
    for (const el of chart_container_html.children) {
        if (el.id == task.id.toString()) {
            selected_task_html = el as HTMLElement;
            start_task_form(el as HTMLElement);
...
// task [number]
function get_new_task_text(): string {
    let name = "task 1";
    let number = 1;
