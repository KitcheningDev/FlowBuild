...
create_btn.addEventListener('click', (e: MouseEvent) => {
    create_new_recipe();
});

export function send_html_request(type: string, str: string, callback: (json: any) => void, body?: any): any {
...
send_html_request('GET', "https://ks29sd5rn3.execute-api.us-east-1.amazonaws.com/staging/task/task/2", (json: any) => {
    for (const task of json) {
        if ((task['SK'] as string).startsWith('TASK')) {
            tasks.push(new Task(task['body'], get_cook((task['cookId'] as number).toString()), new Set(), task['duration']));
        }
    }
    console.log(tasks);
});
