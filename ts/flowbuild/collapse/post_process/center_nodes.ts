...
function local_eval(node: Node, grid: FlowGrid, metric_grid: MetricGrid, graph: Graph): number {
    let length = 0;
...
function set_best_x(node: Node, grid: FlowGrid, metric_grid: MetricGrid, graph: Graph, rules: Set<Rule>): boolean {
    log_grid(grid);
    const old_coords = grid.get_node_coords(node);
    grid.remove_node(node);
    let best_eval = Infinity;
    let best_x = null as number;
...
export function local_center(flow_grid: FlowGrid, metric_grid: MetricGrid, graph: Graph, rules: Set<Rule>): void {
    log_grid(flow_grid);
