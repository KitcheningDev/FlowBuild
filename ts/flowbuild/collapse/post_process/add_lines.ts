...
export function add_ver_line(from: Vec2, y_off: number, grid: FlowGrid): void {
    if (0 < y_off) {
...
function add_normal_line(from: Vec2, to: Vec2, grid: FlowGrid): void {
    if (to.y - from.y == 1) {
        ...
        if (!grid.hor_path_every(from.x, to.x, to.y - 1, (entry: Entry) => !entry.tile.is_solid())) {
            grid.insert_row(to.y);
            return add_normal_line(from, to.down(), grid);
        }
...
function add_loop_bottom_line(from: Vec2, to: Vec2, grid: FlowGrid): void {
...
export function add_lines(grid: FlowGrid, graph: Graph): void {
    // sync_line lines
    ...
    const line_map = new Map<Node | SyncLine, Set<Node | SyncLine>>();
