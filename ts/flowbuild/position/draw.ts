...
function createNew(cook: Cook): HTMLElement {
    const el = html.appendChilds(html.createDiv('cook-new', 'selected'), html.create('i', 'fa-solid', 'fa-plus'));
    el.id = 'new-' + cook.name;
    return el;
}
...
export function draw_grid(flow_grid: FlowGrid, metric_grid: MetricGrid, graph: Graph): void {
    ...
    for (const [tile, coords] of flow_grid.entries) {
        ...
        if (tile.node && tile.node.task && tile.node.task.description != '') {
            if (tile.node.isStart() && graph.cookCount == 1) {
                drawElement(createNew(Cook1), new MetricTile(new Vec2(metric.pos.x, metric.pos.y - metric.withMargin.y / 2 - 15)));
            }
            // task
        ...
        if (tile.cook_title) {
            ...
            if (tile.cook_title.includes('lehrling')) {
                drawElement(createNew(Cook1), new MetricTile(new Vec2(metric.pos.x - metric.withMargin.x / 2 - 30, metric.pos.y)));
            }
            else if (tile.cook_title.includes('meister')) {
                drawElement(createNew(Cook2), new MetricTile(new Vec2(metric.pos.x + metric.withMargin.x / 2 + 30, metric.pos.y)));
            }
        }
        // // carets
    ...
    for (const [node, _] of flow_grid.nodeEntries) {
        if (node.hasTopSyncline) {
            ...
            drawElement(createSyncLine('middle'), new MetricTile(new Vec2((left + right) / 2, y), syncline_tile.dim, syncline_tile.margin), new Vec2(Math.max(right - left, 100), null));
        }
