...
class RecipeEditor {
    ...
    hideSearchingMessage() {
        ...
        const searchingMessage = chatMessages.querySelector(".chat-searching-message");
    ...
    async sendMessageBot(event) {
        event.preventDefault();
        let message = html.recipeBot.messageInput.value.trim();
        var selectedRadio = document.querySelector('input[name="chat-category"]:checked');
        if (message !== "") {
            this.appendMessage(message);
            html.recipeBot.messageInput.value = "";
            this.showSearchingMessage();
            console.log("Selected category: " + (selectedRadio === null || selectedRadio === void 0 ? void 0 : selectedRadio.value));
            if (selectedRadio) {
    ...
    addIngredientRow(ingredient = new Ingredient(new Product("product"), 0, new Unit("unit"), "categorie")) {
        ...
        categories.forEach((element) => {
            ...
            if (sessionStorage.getItem("search")) {
                ...
                const isProductInCategory = search.some((searchCategory) => searchCategory.products.some((product) => product.grocerie === ingredient.product.name &&
    ...
    showConnectTrash() {
        ...
        this.hideNew();
        (_a = document
            .getElementById("connector-" + this.from.id)) === null || _a === void 0 ? void 0 : _a.classList.add("selected");
    ...
    hideConnectTrash() {
        ...
        this.hideNew();
        (_a = document
            .getElementById("connector-" + this.from.id)) === null || _a === void 0 ? void 0 : _a.classList.remove("selected");
        (_b = document
            .getElementById("trash-" + this.from.id)) === null || _b === void 0 ? void 0 : _b.classList.remove("selected");
    }
    showAddRemove() {
        this.hideConnectTrash();
        this.hideNew();
