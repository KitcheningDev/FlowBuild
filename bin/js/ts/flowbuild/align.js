import { setEqual, setIntersection } from "../utils/set.js";
...
function isCookOrderingValid(row) {
...
function matchesHor(left, right) {
...
function dockableHor(tree1, tree2) {
    let docking = NaN;
...
function dockableVer(tree1, tree2) {
    let first_index = NaN;
...
// merge
function mergeHor(left, right) {
    return [...left, ...right.slice(cut(left, right).length)];
}
function dockHor(tree1, tree2, where) {
    const out = [];
    if (where < 0) {
...
function dockVer(tree1, tree2, where) {
    let out = [];
...
function replaceHor(lists, left, right, where) {
    return [...lists, dockHor(left, right, where)].filter((val) => val != left && val != right);
}
function replaceVer(lists, top, bottom, where) {
    return [...lists, dockVer(top, bottom, where)].filter((val) => val != top && val != bottom);
}
    ...
    for (const tree1 of trees) {
        for (const tree2 of trees) {
            ...
            if (overlaps(tree1, tree2)) {
                ...
                if (!isNaN(docking_right) && isvalid(dockHor(tree1, tree2, docking_right))) {
                    return rigidResolve(replaceHor(trees, tree1, tree2, docking_right));
                }
                const docking_left = dockableHor(tree2, tree1);
                if (!isNaN(docking_left) && isvalid(dockHor(tree2, tree1, docking_left))) {
                ...
                if (!isNaN(docking_bottom) && isvalid(dockVer(tree1, tree2, docking_bottom))) {
                ...
                if (!isNaN(docking_top) && isvalid(dockVer(tree2, tree1, docking_top))) {
    ...
    const tryResolve = () => {
        ...
        if (resolved) {
            ...
            for (const tree of resolved) {
                out = dockHor(out, tree, depth(tree[0][0]));
            }
            return true;
        }
        else {
...
function isMostLeft(node, dependancies, exclude = new Set()) {
...
function canCollapse(node, dependancies, order) {
    // old
    let queue = [node];
    const exclude = new Set(order);
    // validate queue
    ...
    let iteration = 0;
    let threshold = 2;
    while (iteration < threshold && queue.length && permuate(queue, validate_queue)) {
        // fill queue
        const temp = [];
...
function collapse(nodes, dependancies) {
    const order = [];
    while (nodes.size) {
        let found = false;
        if (order.length) {
            const back = order[order.length - 1];
            for (const successor of [...back.parents, ...back.childs]) {
                if (!order.includes(successor)) {
                    if (canCollapse(successor, dependancies, order)) {
                        order.push(successor);
                        nodes.delete(successor);
                        found = true;
                        break;
                    }
                }
...
function existsDependancy(dependancy, lookup) {
...
export function createDependancies(graph) {
    const dependancies = [];
    for (const node of graph.nodes) {
        if (1 < node.parents.size && !existsDependancy(node.parents, dependancies)) {
            dependancies.push([...node.parents]);
        }
        if (1 < node.childs.size && !existsDependancy(node.childs, dependancies)) {
            dependancies.push([...node.childs]);
        }
    }
    for (const loop of graph.loops) {
...
export function createGridNew(graph) {
    let order = [];
    const dependancies = createDependancies(graph);
    const callback = (dependancy) => {
        ...
        return permuate(dependancies[index], callback);
    };
    ...
    if (permuate(dependancies[0], callback)) {
