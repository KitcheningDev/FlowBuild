...
class Server {
  ...
  loadRecipes() {
    ...
    this.httpReq(
      "GET",
      "https://ks29sd5rn3.execute-api.us-east-1.amazonaws.com/staging/task",
      (list) => {
        ...
        this.httpReq(
          "GET",
          "https://7y0a1sogrh.execute-api.us-east-1.amazonaws.com/staging/recipe",
          (list) => {
            for (const json of list) {
              this.loadEntry(json);
            }
  ...
  updateSearchResult() {
    ...
    for (const recipe of result) {
      ...
      const item = html.appendChilds(
        html.createDiv("search-item", "btn"),
        img,
        title
      );
      item.onclick = () => {
