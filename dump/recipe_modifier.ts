...
import { Cook1, Task } from "../flowbuild/recipe/task.js";
...
export class RecipeModifier {
    ...
    constructor(recipe: Recipe) {
        this.recipe = recipe;
        this.log = [new RecipeDiff(this.recipe.createGraph())];
    }
    ...
    do_change(): void {
        ...
        for (const [parent, childs] of this.curr_diff.added) {
            for (const child of childs) {
                this.recipe.addConn(parent, child);
            }
        }
        for (const [parent, childs] of this.curr_diff.removed) {
            for (const child of childs) {
                this.recipe.removeConn(parent, child);
            }
        }
        this.log.push(new RecipeDiff(this.recipe.createGraph()));
    }
    undo_change(): void {
    ...
    add_task_from_start(task: Task): void {
        this.add_task(this.recipe.start, task);
    }
    add_task(parent: Task, task: Task): void {
        this.add_task_between(parent, task, this.recipe.end);
    }
    ...
    needs_end(task: Task): boolean {
        for (const child of this.recipe.childs(task)) {
            if (child != this.recipe.end) {
                if (child.cook == task.cook) {
                    const task_node = this.curr_diff.graph.byTask(task);
                    const child_node = this.curr_diff.graph.byTask(child);
                    if (this.curr_diff.graph.isBackwards(child_node)) {
                        return false;
                    }
                }
            }
    ...
    handle_edit(from: Task | null, to: Task | null, cmd: 'add' | 'remove', preferred_cook: string): boolean {
        if (from === null || from == this.recipe.end) {
            return false;
        }

        if (to === null) {
            if (cmd == 'add') {
                const cook = preferred_cook == '0' ? (from.cook ? Cook1 : from.cook) : get_cook(preferred_cook);
                ...
                if (parents.size == 1 && childs.size == 1 && sample(parents) == this.recipe.get_start() && sample(childs) == this.recipe.get_last_step()) {
            ...
            if (cmd == 'add') {
                ...
                // add conn to start
                if (this.curr_diff.graph.byTask(to).parents.size == 1) {
                    if (from == this.recipe.start) {
                        return false;
                    }
                    this.add_connection(this.recipe.start, to);
                }
